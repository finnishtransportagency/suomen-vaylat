            ...
            {identification.citation.date.date &&
                <React.Fragment key={'metadata-dialog-citation-date-content'}>
                    ...
                    <StyledParagraph title={(strings.metadata.codeLists['gmd:CI_DateTypeCode'][identification.citation.date.dateType] || { description: identification.citation.date.dateType }).description}>
                        <div>{moment(identification.citation.date.date).tz('Europe/Helsinki').format('DD.MM.YYYY HH:mm')}</div> ({(strings.metadata.codeLists['gmd:CI_DateTypeCode'][identification.citation.date.dateType] || { description: identification.citation.date.dateType }).label})
                    </StyledParagraph>
                </React.Fragment>
            }
        </React.Fragment>
