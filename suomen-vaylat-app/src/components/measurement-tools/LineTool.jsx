...
export const LineTool = () => {
    ...
    const startLineTool = () => {
        if (!isDrawing) {
            var clearData = ['sv-measure-LineString', true];
            channel.postRequest('DrawTools.StopDrawingRequest', clearData);

            var data = ['sv-measure-LineString', 'LineString', { showMeasureOnMap: true }];
            channel.postRequest('DrawTools.StartDrawingRequest', data);
        } else {
            var data = ['sv-measure-LineString'];
