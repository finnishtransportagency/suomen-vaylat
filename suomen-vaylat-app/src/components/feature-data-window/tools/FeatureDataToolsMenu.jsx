...
const GfiToolsMenu = ({ handleGfiToolsMenu, closeButton = true }) => {
  ...
  const { channel, selectedLayers, gfiLocations, selectedLayersByType } =
  ...
  const [showToast, setShowToast] = useState(
    JSON.parse(localStorage.getItem(DRAWING_TIP_LOCALSTORAGE))
  );
  ...
  const handleSelectDrawingTool = (id, item) => {
    if (id !== selectedGfiTool) {
      ...
      if (
        showToast !== false &&
        !hasToastBeenShown.includes('measurementToast')
      ) {
        if (item.type === 'LineString' || item.type === 'Polygon') {
          toast.info(
            <DrawingToast
              text={strings.tooltips.drawingTools.drawingToast}
  ...
  const handleActivateSavedGeometry = async (features) => {
    ...
    if (features.data[0].name === 'DrawingEvent') {
      ...
      const loaderLength =
        fetchableLayers.length * features.data[0].geojson.features.length;
      ...
      try {
        for (const layer of fetchableLayers) {
          ...
          if (isUserLayer) {
            try {
              // Await the new wrapper for vector layers
      ...
      const loaderLength =
        fetchableLayers.length * features.data[0].data.geom.features.length;
  ...
  const featureEventHandler = async (data) => {
    if (data.operation === 'click') {
      if (data.features) {
        ...
        const loaderLength =
          fetchableLayers.length * data.features[0].geojson.features.length;
      ...
      channel.getFeaturesByGeoJSON(
        ...
        (gfiData) => {
          ...
          setNumberedLoader((prevState) => {
            return prevState
              ? { current: prevState.current + 1, total: prevState.total }
              : prevState;
          });
          resolve('ok');
        },
      ...
      channel.handleEvent('DrawingEvent', async (data) => {
        if (store.getState().ui.selectedGfiTool) {
          if (isSubscribed && data.isFinished && data.isFinished === true) {
            ...
            const fetchableLayers = selectedLayers.filter(
              (layer) =>
                layer.groups?.every((group) => group !== 1) &&
            ...
            try {
              for (const layer of fetchableLayers) {
                const isUserLayer =
                  ...
                  layer.id.startsWith('userlayer_');
                if (isUserLayer) {
                  try {
                    // Use the vector wrapper for user layers
                    await fetchVectorFeaturesSynchronous(
      ...
      <AnimatePresence>
        {isGfiLoading && (
          ...
          >
            ...
            {numberedLoader && (
              <>
                {strings.gfi.loadingData} {numberedLoader.current} /{' '}
                {numberedLoader.total}{' '}
              </>
            )}
          </StyledLoaderWrapper>
        )}
          ...
          {activeSelectionTool === 505 && (
            ...
            >
              {geometries.map((geometry) => {
                return (
                  <StyledSavedView
                    key={geometry.name + (geometry.saveDate || '')}
                    onClick={(e) => {
                      e.preventDefault();
          ...
          gfiCroppingTypes.map((croppingType) => {
            return (
              <CircleButtonListItem
                ...
                item={croppingType}
                icon={
                  icons[croppingType.id]
                    ? icons[croppingType.id].icon
                    : defaultIcon
                }
